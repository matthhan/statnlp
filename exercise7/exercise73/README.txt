...
To do POS-tagging, we use a membership model and a pos bigram model. We train the membership model ourselves, using a simple smoothing method, while the pos bigram model is trained using the SRI toolkit and smoothed with the Kneser Ney method. Finally, the dynamic programming is carried out in the PosTagger class. Inour benchmarks, we reached an error rate of roughly 10%. A ``greedy'' method without dynamic programming that we also implemented achieved an error rate of roughly 12%.

Steps to execute
1. Execute the Makefile
2. Create the language model with the script "make_language_model.sh"
